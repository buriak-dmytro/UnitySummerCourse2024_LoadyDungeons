...
public class ProfileVariableTester
{
    ...
    private static void TestProfileVariable()
    {
        ...
        var variables =
            new ProfileVariables(profileSettings.GetVariableNames());

        for (int i = 0; i < variables.count; i++)
        {
            string variableNames = variables.names[i];
            var value =
                profileSettings.GetValueByName(
                    activeProfileID,
                    variableNames);
            variables.values.Add(value);

            var editorValue =
                profileSettings.EvaluateString(
                    activeProfileID,
                    value);
            ...
            var runtimeValue =
                AddressablesRuntimeProperties.EvaluateString(
                    editorValue);
